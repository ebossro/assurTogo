                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-12">
                                <div class="bg-light p-3 rounded-3 d-flex justify-content-between align-items-center">
                                    ...
                                    <span
                                        class="fw-bold text-primary fs-4">{{ number_format($police->primeMensuelle, 0, ',', ' ') }}
                                        FCFA</span>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        @foreach($police->beneficiaires as $beneficiaire)
                                            <tr>
                                                ...
                                                <td>{{ $beneficiaire->date_naissance ? \Carbon\Carbon::parse($beneficiaire->date_naissance)->format('d/m/Y') : '-' }}
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                        ...
                        @if($police->sinistres->count() > 0)
                            <div class="list-group list-group-flush">
                                @foreach($police->sinistres->take(3) as $sinistre)
                                    <a href="{{ route('sinistres.show', $sinistre) }}"
                                        class="list-group-item list-group-item-action border-0 px-0 py-3">
                                        <div class="d-flex w-100 justify-content-between align-items-center">
                                            <div>
                                                <h6 class="mb-1 fw-bold">{{ $sinistre->reference }}</h6>
                                                <small class="text-muted">{{ $sinistre->date_sinistre->format('d/m/Y') }} -
                                                    {{ Str::limit($sinistre->description, 50) }}</small>
                                            </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card border-0 shadow-sm rounded-4 text-white bg-primary bg-gradient">
                    <div class="card-body p-4 text-center">
                        ...
                        <p class="small mb-4 text-white-50">Notre équipe est disponible pour répondre à toutes vos questions
                            concernant votre police d'assurance.</p>
                        <a href="https://wa.link/0xo0yi" target="_blank" class="btn btn-light text-primary fw-bold w-100">
